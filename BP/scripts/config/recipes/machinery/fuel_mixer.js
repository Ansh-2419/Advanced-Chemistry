...
function defineFuelMixerRecipe(recipe) {
    if (!recipe || typeof recipe !== "object")
        throw new TypeError("Invalid fuel mixer recipe");
    return {
        id: recipe.id ?? `${recipe.input1.type}_${recipe.input2.type}`,
        input1: {
            type: recipe.input1.type.toLowerCase(),
            amount: Math.max(1, recipe.input1.amount ?? 500)
        },
        input2: {
            type: recipe.input2.type.toLowerCase(),
            amount: Math.max(1, recipe.input2.amount ?? 500)
        },
        output: {
            type: recipe.output.type.toLowerCase(),
            amount: Math.max(1, recipe.output.amount ?? 800)
        },
...
system.afterEvents.scriptEventReceive.subscribe(({ id, message }) => {
    ...
    try {
        ...
        for (const [recipeId, definition] of Object.entries(payload)) {
            try {
                const recipe = defineFuelMixerRecipe({
                    id: recipeId,
                    ...definition
                });
                const idx = fuelMixerRecipes.findIndex(r => r.id === recipe.id);
                if (idx >= 0) {
                    fuelMixerRecipes[idx] = recipe;
                    replaced++;
                } else {
                    fuelMixerRecipes.push(recipe);
                    added++;
                }
            } catch (err) {
                console.warn(
                    `[UtilityCraft] Failed to register fuel mixer recipe '${recipeId}':`,
                    err
                );
            }
        }
        console.warn(
            `[UtilityCraft] Fuel Mixer: registered ${added} new, replaced ${replaced} recipes.`
        );
    } catch (err) {
        console.warn(
            "[UtilityCraft] Failed to parse fuel mixer recipe payload:",
            err
        );
    }
});
