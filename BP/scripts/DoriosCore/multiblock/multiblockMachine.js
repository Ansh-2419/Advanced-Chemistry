...
export class MultiblockMachine extends BasicMachine {
    ...
    static defaultOnInteractWithoutWrench({ entity, player }) {
        if (!entity) return;
        player.sendMessage(
            "§7Use a wrench to scan and activate this multiblock."
        );
    }
    ...
    constructor(block, config) {
        ...
        const state = this.entity.getDynamicProperty(
            Constants.STATE_PROPERTY_ID
        );
    ...
    static spawnEntity(e, config, callback) {
        ...
        system.run(() => {
            const entity = Utils.spawnEntity(block, {
                ...config,
                spawn_offset: { x: 0, y: -0.5, z: 0 }
            });
    ...
    static async handlePlayerInteract(e, config, handlers = {}) {
        ...
        const entity = block.dimension.getEntitiesAtBlockLocation(
            block.location
        )[0];
        ...
        if (!isUsingWrench) {
            return onInteractWithoutWrench?.({
                e,
                entity,
                player,
                config,
        ...
        const activate = targetEntity =>
            this.activateMachineController(e, config, targetEntity, {
                onActivate,
                successMessages
            });
        ...
        if (!entity) {
            this.spawnEntity(e, config, spawnedEntity => {
                initializeEntity?.(spawnedEntity, {
                    e,
                    player,
                    config,
                    settings: config
        ...
        system.run(() => {
            if (player?.isInSurvival()) {
                const oldItemEntity = dim
                    ...
                    .find(
                        item =>
                            item.getComponent("minecraft:item")?.itemStack
                                ?.typeId === blockItemId
                    );
    ...
    static async activateMachineController(e, config, entity, handlers = {}) {
        const { onActivate, successMessages = [] } = handlers;
        const { block, player } = e;
        const requirements = config.requirements ?? {};

        DeactivationManager.deactivateMultiblock(block, player);

        const structure = await StructureDetector.detectFromController(
            e,
            config.required_case
        );
        if (!structure) return;

        const failure = this.validateRequirements(
        ...
        const energyCap = ActivationManager.activateMultiblock(
            entity,
            structure
        );
        ...
        const messages =
            typeof successMessages === "function"
                ? successMessages(context)
                : successMessages;
    ...
    displayProgress(options = {}) {
        const energyCost =
            options.maxValue ?? this.getEnergyCost(options.index);
    ...
    setEnergyCost(value, index = 0) {
        this.entity.setDynamicProperty(
            `${MachineryConstants.MACHINE_ENERGY_COST_PROPERTY_PREFIX}${index}`,
            Math.max(1, value)
        );
    }
    ...
    getEnergyCost(index = 0) {
        return (
            this.entity.getDynamicProperty(
                `${MachineryConstants.MACHINE_ENERGY_COST_PROPERTY_PREFIX}${index}`
            ) ?? MachineryConstants.DEFAULT_PROGRESS_MAX
        );
    }
    ...
    static computeMachineStats(components) {
        ...
        const speedPenalty =
            1 + (maxSpeedPenalty * speed) / (speedPenaltyK + speed);

        const minEfficiency = 0.01;
        const efficiencyRate = 0.15;
        const efficiencyMultiplier =
            minEfficiency +
            (1 - minEfficiency) * Math.exp(-efficiencyRate * efficiency);
        ...
        return {
            ...
            energyMultiplier:
                processingPenalty * speedPenalty * efficiencyMultiplier
        };
    }
