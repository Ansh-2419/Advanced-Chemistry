...
function normalizeButtons(buttons) {
    const entries = Array.isArray(buttons)
        ? buttons.map((button, index) => [button?.id ?? String(index), button])
...
function itemMatchesButton(item, interfaceDefinition, button) {
    return (
        getItemTypeId(item) === getButtonItemId(interfaceDefinition, button) &&
        decodeInterfaceSlot(item.nameTag) === button.slot
    );
}
...
export function decodeInterfaceSlot(nameTag) {
    if (typeof nameTag !== "string" || nameTag.length < SLOT_CODE_LENGTH)
        return undefined;
    if (
        nameTag[0] !== SLOT_CODE_MARKER ||
        nameTag[2] !== SLOT_CODE_MARKER ||
        nameTag.slice(4, 6) !== SLOT_CODE_RESET
    ) {
        return undefined;
    }
...
export function stripInterfaceSlotCode(nameTag) {
    return decodeInterfaceSlot(nameTag) === undefined
        ? nameTag
        : nameTag.slice(SLOT_CODE_LENGTH);
}
...
export class InterfaceManager {
    ...
    static setButton(container, context) {
        const item = new ItemStack(
            getButtonItemId(context.interface, context.button),
            1
        );
        item.nameTag = this.createSlotNameTag(
            context.slot,
            getButtonNameTag(context)
        );
    ...
    static getEntityButtons(entity, block = tryGetBlockFromEntity(entity)) {
        ...
        for (const interfaceId of getLinkedInterfaceIds(entity, block)) {
            ...
            for (const [
                buttonId,
                button
            ] of interfaceDefinition.buttons.entries()) {
                buttons.push({
                    interfaceId,
                    interfaceDefinition,
                    buttonId,
    ...
    static getInterfaceEntityFromBlock(block) {
        ...
        let entities = [];
        try {
            entities = block.dimension.getEntitiesAtBlockLocation(
                block.location
            );
        } catch {
            return undefined;
    ...
    static handlePlayerButtonDrop(player) {
        if (!isValidEntity(player) || player.typeId !== "minecraft:player")
            return false;
        ...
        return this.handlePressedButtons(player, {
            entity,
            block,
            container,
            pressedButtons
     ...
     */
    static handlePressedButtons(
        player,
        { entity, block, container, pressedButtons }
    ) {
        if (pressedButtons.length !== 1) {
            if (pressedButtons.length > 1) {
                for (const pressed of pressedButtons) {
                    this.setButton(
                        container,
                        getContext({ ...pressed, entity, block, player })
                    );
                }
            }
            return false;
        ...
        try {
            ...
            console.warn(
                `[InterfaceManager] button ${pressed.interfaceId}:${pressed.buttonId} failed: ${error?.message ?? error}`
            );
        }
    ...
    static resolveDroppedButtonFromOpenEntities(player) {
        ...
        for (const entity of ContainerSessionManager.getOpenEntities()) {
            ...
            const pressedButtons = this.getMissingButtons(
                entity,
                block,
                container
            );
    ...
    static getMissingButtons(entity, block, container) {
        ...
        for (const buttonData of this.getEntityButtons(entity, block)) {
            ...
            if (
                itemMatchesButton(
                    current,
                    buttonData.interfaceDefinition,
                    buttonData.button
                )
            )
                continue;
...
world.afterEvents.entityContainerOpened.subscribe(event => {
    InterfaceManager.ensureEntityInterfaces(
        event?.entity,
        event?.player ?? event?.source ?? event?.sourceEntity
    );
});
