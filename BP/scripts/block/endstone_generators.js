DoriosAPI.register.blockComponent("endstone_generators", {
    onTick({ block, dimension }, { params }) {
        let { x, y, z } = block.location

        const amount = Math.max(1, params?.amount ?? 1)
        const material = params?.material ?? "minecraft:end_stone"
        ...
        // Try to insert using amount + accumulated progress
